...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/user/webUser/">用户管理列表</a></li>
		<li class="active"><a href="${ctx}/user/webUser/form?id=${webUser.id}">用户管理<shiro:hasPermission name="user:webUser:edit">${not empty webUser.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="user:webUser:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="webUser" action="${ctx}/user/webUser/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:treeselect id="office" name="office.id" value="${webUser.office.id}" labelName="office.name" labelValue="${webUser.office.name}"
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="loginDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${webUser.loginDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<div class="form-actions">
			<shiro:hasPermission name="user:webUser:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>

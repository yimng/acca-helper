		...
		<c:forEach items="${page.list}" var="user">
			...
			<td>${user.loginName}</td>
				...
				<shiro:hasPermission name="sys:user:edit"><td>
					<c:if test="${user.id !='03135bed43154a1eafcb8facda6dfad5' }">
	    				<a href="${ctx}/sys/user/form?id=${user.id}">修改</a>
						<a href="${ctx}/sys/user/delete?id=${user.id}" onclick="return confirmx('确认要删除该用户吗？', this.href)">删除</a>
						<!-- 涉及到平台账号问题，未做 -->
						<c:if test="${user.loginFlag == 1}">
							<a href="${ctx}/sys/user/notLogin?id=${user.id}&loginFlag=0" onclick="return confirmx('确认要禁用该账号吗？', this.href)">禁用账号</a>
						</c:if>
						<c:if test="${user.loginFlag == 0}">
							<a href="${ctx}/sys/user/notLogin?id=${user.id}&loginFlag=1" onclick="return confirmx('确认要恢复该账号吗？', this.href)">恢复账号</a>
						</c:if>
						<a href="${ctx}/sys/user/resetPass?id=${user.id}" onclick="return confirmx('确认要将该用户密码重置为123456吗？', this.href)">重置密码</a>
					</c:if>

...
import com.caicui.smsChuanglan.sender.SmsSenderChuanglan;
import com.caicui.smsChuanglan.util.ChuangLanResultHelper;
...
public class SmsUtils {
	...
	// public static HashMap<String, Object> sendSmsVcode(String phone, String
	...
	public static HashMap<String, Object> sendSmsVcode(String phone, String[] params) {
		 ...
		 * Object>) result.get("data");//正常返回输出data包体信息（map） Set<String> keySet =
		 * data.keySet(); for(String key:keySet){ Object object = data.get(key);
		 * System.out.println(key +" = "+object); } return true; }else{ //异常返回输出错误码和错误信息
		 * statusCode = (String) result.get("statusCode"); statusMsg = (String)
		 * result.get("statusMsg"); System.out.println("错误码=" + result.get("statusCode")
		 * +" 错误信息= "+result.get("statusMsg")); logger.warn("send sms vcode , error:" +
		 * "错误码=" + result.get("statusCode") +" 错误信息= "+result.get("	")); return
		 * false; }
		 */
	}
	...
	public static HashMap<String, Object> sendSms2Vcode(String phone, String msg) {
		...
		String returnString = "";
		try {
			// returnString = HttpSender.batchSend(url, account, password, phone, msg,
			// needstatus, "",extno);
			returnString = SmsSenderChuanglan.getSmsSender().send(url, account, password, phone, msg);
		...
		if (!StringUtils.isEmpty(returnString)) {
			...
			Map<String, String> translateResultMap = ChuangLanResultHelper.getInstance().translateJson(returnString);

			String statusCode = translateResultMap.get(ChuangLanResultHelper.RESPONSE_STATUS);
	...
	public static HashMap<String, Object> sendSms2Vcode(String phone, String[] params) {
		...
		try {
			// returnString = HttpSender.batchSend(url, account, pswd, phone, result,
			// needstatus, "", extno);
			returnString = SmsSenderChuanglan.getSmsSender().send(url, account, password, phone, result);
